...
simudata.para.setting <- function(number.factors,  number.items, correlation.factors,
  ...
  if(number.factors==2){
    ...
    #model syntax
    simu.f1 <- rep(c(1,0), each=(number.items*(1/2)))
    simu.f2 <- rep(c(0,1), each=(number.items*(1/2)))
    simu.qm <- data.frame(simu.f1, simu.f2)
    simu.qm <- as.matrix(simu.qm) #set slop parameter
    ...
    #model syntax
    simu.f1 <- rep(c(1,0,0,0), each=(number.items*(1/4)))
    simu.f2 <- rep(c(0,1,0,0), each=(number.items*(1/4)))
    simu.f3 <- rep(c(0,0,1,0), each=(number.items*(1/4)))
    simu.f4 <- rep(c(0,0,0,1), each=(number.items*(1/4)))
    simu.qm <- data.frame(simu.f1, simu.f2, simu.f3, simu.f4)
    simu.qm <- as.matrix(simu.qm)
    ...
    #model syntax
    simu.f1 <- rep(c(1,0,0,0,0,0), each=(number.items*(1/6)))
    simu.f2 <- rep(c(0,1,0,0,0,0), each=(number.items*(1/6)))
    simu.f3 <- rep(c(0,0,1,0,0,0), each=(number.items*(1/6)))
    simu.f4 <- rep(c(0,0,0,1,0,0), each=(number.items*(1/6)))
    simu.f5 <- rep(c(0,0,0,0,1,0), each=(number.items*(1/6)))
    simu.f6 <- rep(c(0,0,0,0,0,1), each=(number.items*(1/6)))
    simu.qm <- data.frame(simu.f1, simu.f2, 
                          simu.f3, simu.f4,
                          simu.f5, simu.f6)
    simu.qm <- as.matrix(simu.qm)
...
saveRDS(total.simudata, file = 'C:/Users/u0124272/OneDrive - KU Leuven/Desktop/be84_gdina_simudata_100.rds')
