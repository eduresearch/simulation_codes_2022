...
simudata.para.setting <- function(number.factors,  number.items, correlation.factors,
  ...
  if(number.factors==2){
    ...
    #model syntax
    cfa.syntax <- paste('0.55*Item_', 1:number.items, sep = '')
    cfa.mul2.f1 <- paste('f1 =~', '0.55*Item_1', paste('+', cfa.syntax[2:(number.items*(3/4))], sep = '', collapse=' '))
    cfa.f2.1st <- paste('0.55*Item_', (number.items*(1/4)+1), sep = '')
    cfa.mul2.f2 <- paste('f2 =~', cfa.f2.1st, paste('+', cfa.syntax[((number.items*(1/4)+2)):number.items], sep = '', collapse=' '))
    ...
    #model syntax
    cfa.syntax <- paste('0.25*Item_', 1:number.items, sep = '')
    cfa.mul.wi <- paste('+', cfa.syntax[(number.items*(2/4)+1):(number.items*(3/4))],  sep = '', collapse=' ')
    cfa.mul4.f1 <- paste('f1 =~', '0.25*Item_1', 
                         paste('+', cfa.syntax[2:(number.items*(1/4))], sep = '', collapse=' '))
    cfa.mul4.f1 <- paste(cfa.mul4.f1, cfa.mul.wi, sep = '', collapse=' ')
    
    cfa.f2.1st <- paste('0.25*Item_', (number.items*(1/4))+1, sep = '')
    cfa.f3.1st <- paste('0.25*Item_', (number.items*(2/4))+1, sep = '')
    cfa.f4.1st <- paste('0.25*Item_', (number.items*(2/4))+1, sep = '')
    
    cfa.mul4.f2 <- paste('f2 =~', cfa.f2.1st, 
                         paste('+', cfa.syntax[((number.items*(1/4))+2):(number.items*(3/4))], sep = '', collapse=' '))
    ...
    cfa.mul4.f4 <- paste('f4 =~', cfa.f4.1st, 
                         paste('+', cfa.syntax[((number.items*(2/4))+2):number.items], sep = '', collapse=' '))
    ...
    #model syntax
    cfa.syntax <- paste('0.15*Item_', 1:number.items, sep = '')
    cfa.mul.wi <- paste('+', cfa.syntax[(number.items*(4/6)+1):(number.items*(5/6))],  sep = '', collapse=' ')
    
    cfa.mul6.f1 <- paste('f1 =~', '0.15*Item_1', 
                         paste('+', cfa.syntax[2:(number.items*(1/6))], sep = '', collapse=' '))
    cfa.mul6.f1 <- paste(cfa.mul6.f1, cfa.mul.wi, sep = '', collapse=' ')
    
    cfa.f2.1st <- paste('0.15*Item_', (number.items*(1/6))+1, sep = '')
    cfa.f3.1st <- paste('0.15*Item_', (number.items*(2/6))+1, sep = '')
    cfa.f4.1st <- paste('0.15*Item_', (number.items*(3/6))+1, sep = '')
    cfa.f5.1st <- paste('0.15*Item_', (number.items*(4/6))+1, sep = '')
    cfa.f6.1st <- paste('0.15*Item_', (number.items*(5/6))+1, sep = '')
    
    cfa.mul6.f2 <- paste('f2 =~', cfa.f2.1st, 
                         paste('+', cfa.syntax[((number.items*(1/6))+2):(number.items*(2/6))], sep = '', collapse=' '))
    cfa.mul6.f2 <- paste(cfa.mul6.f2, cfa.mul.wi, sep = '', collapse=' ')
    
    cfa.mul6.f3 <- paste('f3 =~', cfa.f3.1st, 
                         paste('+', cfa.syntax[((number.items*(2/6))+2):(number.items*(3/6))], sep = '', collapse=' '))
    cfa.mul6.f3 <- paste(cfa.mul6.f3, cfa.mul.wi, sep = '', collapse=' ')
    
    cfa.mul6.f4 <- paste('f4 =~', cfa.f4.1st, 
                         paste('+', cfa.syntax[((number.items*(3/6))+2):(number.items*(4/6))], sep = '', collapse=' '))
    cfa.mul6.f4 <- paste(cfa.mul6.f4, cfa.mul.wi, sep = '', collapse=' ')
    ...
    cfa.mul6.f6 <- paste(cfa.mul6.f6, cfa.mul.wi, sep = '', collapse=' ')
...
total.simudata[[162]]
total.simudata[[162]]$data[[1]]
saveRDS(total.simudata, file = 'C:/Users/u0124272/OneDrive - KU Leuven/Desktop/wi84_cfa_simudata_100.rds')
