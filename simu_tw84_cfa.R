...
simudata.para.setting <- function(number.factors,  number.items, correlation.factors,
  ...
  if(number.factors==2){
    ...
    cfa.mul2.g1 <- paste('g1 =~', 'Item_1', paste('+', cfa.syntax[2:(number.items/2)], sep = '', collapse=' '))
    cfa.g2.1st <- paste('Item_', (number.items/2)+1, sep = '')
    cfa.mul2.g2 <- paste('g2 =~', cfa.g2.1st, paste('+', cfa.syntax[((number.items/2)+2):number.items], sep = '', collapse=' '))
    ...
    cfa.mul2.cov <- 'g1 ~~ 0*f1 \n g1 ~~ 0*f2 
                     g2 ~~ 0*f1 \n g2 ~~ 0*f2
                     g1 ~~ g2
                     f1 ~~ 0*f2' 
    cfa.mul.tw <- paste(cfa.mul2.g1, cfa.mul2.g2, cfa.mul2.f1, cfa.mul2.f2, cfa.mul2.cov, sep = '\n') #set line break to each factor
    ...
    #model syntax
    cfa.syntax <- paste('Item_', 1:number.items, sep = '')
    cfa.mul4.g1 <- paste('g1 =~', 'Item_1', paste('+', cfa.syntax[2:(number.items/2)], sep = '', collapse=' '))
    cfa.g2.1st <- paste('Item_', (number.items/2)+1, sep = '')
    cfa.mul4.g2 <- paste('g2 =~', cfa.g2.1st, paste('+', cfa.syntax[((number.items/2)+2):number.items], sep = '', collapse=' '))
    ...
    cfa.mul.tw <- paste(cfa.mul4.g1, cfa.mul4.g2, 
                        cfa.mul4.f1, cfa.mul4.f2, 
                        cfa.mul4.f3, cfa.mul4.f4, cfa.mul4.cov,
                        sep = '\n') 
  }
  else{
    factors <- number.factors
    ...
    #model syntax
    cfa.syntax <- paste('Item_', 1:number.items, sep = '')
    cfa.mul6.g1 <- paste('g1 =~', 'Item_1', paste('+', cfa.syntax[2:(number.items/2)], sep = '', collapse=' '))
    cfa.g2.1st <- paste('Item_', (number.items/2)+1, sep = '')
    cfa.mul6.g2 <- paste('g2 =~', cfa.g2.1st, paste('+', cfa.syntax[((number.items/2)+2):number.items], sep = '', collapse=' '))
    ...
    cfa.mul.tw <- paste(cfa.mul6.g1, cfa.mul6.g2,
                        cfa.mul6.f1, cfa.mul6.f2, cfa.mul6.f3,
...
saveRDS(total.simudata, file = 'C:/Users/u0124272/OneDrive - KU Leuven/Desktop/tw84_cfa_simudata_100.rds')
